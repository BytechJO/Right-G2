...
// بيانات التمرين
const wordBankWords = ['drum', 'kite', 'sandwich', 'bench', 'pond', 'bike', 'paint'];
...
const DraggableWord = ({ word, isUsed }) => {
    const { attributes, listeners, setNodeRef, transform } = useDraggable({ id: word, data: { word } });
...
const DropZone = ({ id, children, isOver }) => {
    ...
    return (
        <div ref={setNodeRef} className={`w-full border-b-2 pb-1 transition-colors ${isOver ? 'border-blue-400 bg-blue-50' : 'border-gray-300'}`}>
            {children}
        </div>
    );
};
...
const WB_Unit3_Page16_Q1 = () => {
    ...
    const getBorderColor = (qId) => {
        ...
        return placedWords[qId] === correctAnswers[qId] ? 'border-green-500' : 'border-red-500';
    };
        ...
        <DndContext onDragEnd={handleDragEnd} collisionDetection={closestCenter}>
            <div className="p-6 max-w-4xl mx-auto font-sans">
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <h1 className="header-title-page8">Look, read, and label the pictures. Use the words from the box.</h1>
                </div>

                <div className="flex flex-wrap justify-center gap-3 p-3 mb-6 border border-gray-300 rounded-lg">
                    {wordBankWords.map(word => (
                        <DraggableWord key={word} word={word} isUsed={Object.values(placedWords).includes(word)} />
                    ))}
                </div>

                <img src={mainPic} alt="Park scene" className="max-w-full max-h-100 rounded-lg  mb-6 lg:ml-20" />
                ...
                <div className="grid grid-cols-2 gap-x-12 gap-y-6 text-lg">
                    {questionNumbers.map(qId => (
                        <div key={qId} className="flex items-center gap-3">
                            ...
                            <DropZone id={qId}>
                                <div className={`w-full text-center text-xl font-semibold pb-1 transition-colors ${getBorderColor(qId)}`}>
                                    {placedWords[qId] || <span className="text-transparent">.</span>}
                                </div>
                            </DropZone>
                        </div>
                    ))}
