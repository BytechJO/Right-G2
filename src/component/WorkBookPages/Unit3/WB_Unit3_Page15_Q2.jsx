...
const WB_Unit3_Page15_Q2 = () => {
    ...
    const [writtenAnswers, setWrittenAnswers] = useState({ q1: '', q2: '', q3: '', q4: '' });
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto font-sans bg-white rounded-xl">
            <div className="flex items-start gap-4 mb-6">
                <span className="ex-A">B</span>
                <div>
                    <h1 className="header-title-page8">Read and write ✔. Write the question and answer.</h1>
                </div>
            </div>
                ...
                <table className="min-w-full border border-gray-300 text-center">
                    <thead className="bg-gray-100">
                        <tr>
                            ...
                            <th className="p-3 border-r border-gray-300 font-semibold text-gray-700">Yes, I can.</th>
                            <th className="p-3 font-semibold text-gray-700">No, I can't.</th>
                        </tr>
                    </thead>
                    <tbody>
                        {questions.map((q, index) => (
                            <tr key={q.id} className="border-t border-gray-300">
                                ...
                                >
                                    {selections[q.id] === true && <span className="text-2xl text-blue-600 font-bold">✔</span>}
                                </td>
                                <td
                                    onClick={() => handleSelect(q.id, false)}
                                    className={`p-3 cursor-pointer transition-colors ${getCellClass(q.id, false)}`}
                                >
                                    {selections[q.id] === false && <span className="text-2xl text-blue-600 font-bold">✔</span>}
                                </td>
                            </tr>
                        ))}
                    </tbody>
