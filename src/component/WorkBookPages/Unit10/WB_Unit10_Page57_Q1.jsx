...
import imgCooking from "../../../assets/imgs/test6.png";
...
const CORRECT_ANSWERS = {
    q1: 'act1', q2: 'act2', q3: 'act3', q4: 'act4',
    q5: 'act5', q6: 'act6', q7: 'act7', q8: 'act8'
};
...
function DropSlot({ id, content, isCorrect, isSubmitted }) {
    ...
    return (
        <div ref={setNodeRef} className={`w-full min-h-[40px] border-b-2 flex items-center justify-center px-2 transition-all ${borderColor}`}>
            {content ? <span className="text-blue-900 font-bold text-sm text-center">{ACTIVITIES.find(a => a.id === content).text}</span> : <span className="text-gray-300 italic text-xs">Drop answer here...</span>}
        </div>
    );
}

const WB_Unit10_Page57_Q1 = () => {
    const [answers, setAnswers] = useState({ q1: null, q2: null, q3: null, q4: null, q5: null, q6: null, q7: null, q8: null });
    ...
    const handleReset = () => { setAnswers({ q1: null, q2: null, q3: null, q4: null, q5: null, q6: null, q7: null, q8: null }); setShowResults(false); };
        ...
        <DndContext sensors={sensors} onDragStart={(e) => setActiveId(e.active.id)} onDragEnd={(e) => { if (e.over) setAnswers(prev => ({ ...prev, [e.over.id]: e.active.id })); setActiveId(null); }}>
            <div className="p-6 max-w-6xl mx-auto font-sans">
                        ...
                        {QUESTIONS.map(q => (
                            <div key={q.id} className="space-y-3">
                                ...
                                <div className="bg-gray-50 p-2 rounded-xl border border-gray-100 flex justify-center">
                                    <img src={q.img} alt="activity" className="max-h-32 object-contain rounded-lg" />
                                </div>
                                <DropSlot id={q.id} content={answers[q.id]} isCorrect={answers[q.id] === CORRECT_ANSWERS[q.id]} isSubmitted={showResults} />
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="flex-1 bg-blue-50 p-5 rounded-2xl border-2 border-blue-100 h-fit sticky top-4">
                        ...
                        <div className="grid grid-cols-1 gap-2">
                            <SortableContext items={ACTIVITIES.map(a => a.id)}>
                                {ACTIVITIES.map(act => <DraggableActivity key={act.id} item={act} isUsed={Object.values(answers).includes(act.id)} />)}
                            </SortableContext>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-12 flex justify-center">
                    <Button handleShowAnswer={() => { setAnswers(CORRECT_ANSWERS); setShowResults(true); }} handleStartAgain={handleReset} checkAnswers={checkAnswers} />
                </div>
            </div>
