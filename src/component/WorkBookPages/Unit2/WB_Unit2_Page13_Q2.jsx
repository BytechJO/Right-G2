...
const WB_Unit2_Page13_Q2 = () => {
    ...
    const checkAnswers = () => {
        ...
        if (correctCount === exerciseQuestions.length) {
            ValidationAlert.success(`Score: ${correctCount}/${exerciseQuestions.length}`);
        } else {
            ValidationAlert.error(`Score: ${correctCount}/${exerciseQuestions.length}`);
        }
    };
    ...
    const getCheckboxClass = (questionId, option) => {
        ...
        if (showResults) {
            const isCorrect = exerciseQuestions.find(q => q.id === questionId).correctAnswer === option;
            if (isCorrect) {
        ...
        <div className="p-6 max-w-4xl mx-auto font-sans">
            ...
            <div className="space-y-4">
                {exerciseQuestions.map((question, index) => (
                    <div key={question.id} className="grid grid-cols-[auto_1fr_auto] items-center gap-x-6 p-3 rounded-lg hover:bg-gray-50">
                        {/* الصورة */}
                        <img src={question.img} alt={`Question ${index + 1}`} className="max-w-16 max-h-16 object-contain" />
                        ...
                        <div className="flex items-center gap-x-4">
                            ...
                            <div className="flex flex-col items-center">
                                {index === 0 && <span className="font-semibold text-gray-600 mb-1">True</span>}
                                <div
                                ...
                                >
                                    {userAnswers[question.id] === true && <span className="text-2xl text-blue-600">✔</span>}
                                </div>
                            </div>
                            {/* False */}
                            <div className="flex flex-col items-center">
                                {index === 0 && <span className="font-semibold text-gray-600 mb-1">False</span>}
                                <div
                                ...
                                >
                                    {userAnswers[question.id] === false && <span className="text-2xl text-blue-600">✔</span>}
                                </div>
                            </div>
                        </div>
                    </div>
