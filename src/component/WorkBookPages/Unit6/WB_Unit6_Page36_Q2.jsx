...
function QuestionSlot({ id, prefix, suffix, content, isCorrect, isSubmitted }) {
    ...
    const borderColor = isSubmitted ? (isCorrect ? 'border-blue-600 bg-green-50' : 'border-blue-600 bg-blue-50') : (isOver ? 'border-blue-400 bg-blue-50' : 'border-gray-300');
    ...
    return (
        <div className="flex items-center flex-wrap gap-2 text-lg py-2 border-b border-gray-100">
            ...
            >
                {content ? (
                    <span className="text-blue-800 font-bold">{PHRASES.find(p => p.id === content).text}</span>
                ) : (
...
const WB_Unit6_Page36_Q2 = () => {
    const [placed, setPlaced] = useState({ h2: null, h3: null, h4: null, h5: null, h6: null });
   ...
   const handleStartAgain = () => {
    ...
    return (
        <DndContext sensors={sensors} onDragStart={(e) => setActiveId(e.active.id)} onDragEnd={handleDragEnd} >
            ...
            <div className="mb-20 p-8 max-w-4xl mx-auto font-sans" ref={captureRef}>
                <div className="flex items-center gap-4 mb-12">
                    <div className="ex-A">H</div>
                    <h1 className="header-title-page8">Write questions. Use the phrases in the boxes.</h1>
                </div>
                <div className="flex flex-wrap justify-center gap-3 mb-10 p-5 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                    <SortableContext items={PHRASES.map(p => p.id)}>
                        {PHRASES.map(phrase => (
                            <DraggablePhrase key={phrase.id} phrase={phrase} isUsed={Object.values(placed).includes(phrase.id)} />
                        ))}
                    </SortableContext>
                </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-2 text-lg py-2 border-b border-gray-100 opacity-60">
                        ...
                        <span className="px-2 border-b-2 border-gray-300 font-bold text-blue-800">brush his teeth</span>
                        <span>?</span>
                    </div>

                    <QuestionSlot id="h2" prefix="What time does she" suffix="?" content={placed.h2} isCorrect={placed.h2 === CORRECT_ANSWERS.h2} isSubmitted={isSubmitted} />
                    <QuestionSlot id="h3" prefix="What time do" suffix="they get up?" content={placed.h3} isCorrect={placed.h3 === CORRECT_ANSWERS.h3} isSubmitted={isSubmitted} />
                    <QuestionSlot id="h4" prefix="What time does" suffix="he go home?" content={placed.h4} isCorrect={placed.h4 === CORRECT_ANSWERS.h4} isSubmitted={isSubmitted} />
                    <QuestionSlot id="h5" prefix="What time does" suffix="she have lunch?" content={placed.h5} isCorrect={placed.h5 === CORRECT_ANSWERS.h5} isSubmitted={isSubmitted} />
                    <QuestionSlot id="h6" prefix="What time do" suffix="they do homework?" content={placed.h6} isCorrect={placed.h6 === CORRECT_ANSWERS.h6} isSubmitted={isSubmitted} />
                </div>
