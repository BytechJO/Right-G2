...
const WB_Unit7_Page44_Q1 = () => {
    const [answersA, setAnswersA] = useState({ 1: null, 2: null, 3: null, 4: null });
    ...
    const checkAnswers = () => {
        ...
        if (correctA === exerciseAData.length) {
            ValidationAlert.success(`Excellent! Score: ${correctA}/${exerciseAData.length}`);
        } else {
            ValidationAlert.error(`Keep trying! Score: ${correctA}/${exerciseAData.length}`);
        }
    };
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto">
            <div className="flex items-center gap-3 mb-8">
                ...
                <h2 className="header-title-page8">Listen and write ✓.</h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
                {exerciseAData.map((item) => (
                    <div key={item.id} className="flex items-center justify-between p-6">
                        <img src={item.image} alt="" className="max-w-40 max-h-32 object-contain" />
                        <div className="flex flex-col gap-3">
                            {item.options.map((opt) => (
                                <div key={opt} className="flex items-center gap-4">
                                    <span className="w-10 text-right font-bold text-lg text-gray-700">{opt}</span>
                                    <div 
                                        ...
                                        className={`w-10 h-10 border-2 border-gray-400 rounded-lg flex items-center justify-center cursor-pointer transition-all
                                            ${answersA[item.id] === opt ? "bg-blue-50 border-blue-500" : "hover:bg-gray-50"}
                                            ${checked && opt === item.correct ? "bg-green-100 border-green-500" : ""}
                                            ${checked && answersA[item.id] === opt && opt !== item.correct ? "bg-red-100 border-red-500" : ""}
                                        `}
                                    >
                                        {answersA[item.id] === opt && <span className="text-2xl font-bold text-blue-600">✓</span>}
                                        {checked && opt === item.correct && answersA[item.id] !== opt && <span className="text-2xl font-bold text-green-600 opacity-40">✓</span>}
                                    </div>
                                </div>
                            ))}
                        </div>
